...
Variabelen aangemaakt buiten functies noemen we "globaal", en zijn beschikbaar op alle plaatsen in jouw python script. Bovenstaande script zou correct werken met de volgende aanpassing:
...
Afhankelijk van de programmeertaal die je gebruikt is scope meer of minder complex,  voorlopig werken we met twee "scopes"
...
print("Buiten de functie: " + a)
```
Waarschijnlijk zal jouw IDE al iets van een waarschuwing geven, laten we die even negeren.
...
Een probleem van globale variabelen is dat je al snel het overzicht kwijt raakt. En stel dat je in functie A() een globale variabele wil uitlezen, maar een collega nieuwe code toevoegt die deze in functie B() wijzigt zonder jouw weten. Python probeert je hier te helpen door, tenzij je anders verteld, bij aanpassen van een globale variabele een nieuwe functie scope variabele te maken. Dus de "a" in de functie is eigenlijk een hele andere "a" dan die buiten de functie. Op systeemniveau verwijst deze zelfs naar een ander geheugenadres. 

Wil je nu expliciet een globale variabele wijzigen in een functie, dan dien je dat te melden aan Python met het sleutelwoord "global":
